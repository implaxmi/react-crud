...
const onDelete = async (id) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
     onAdd(e.target.name.value,e.target.capacity.value, e.target.status.value, e.target.image.value);
     e.target.name.value = "";
...
const onEdit = async (id, name, capacity, status, image) => {
  ...
  return (
    ...
    <table>
      ...
      {users.map((user) => (
        <User key={user.id} id={user.id} name={user.name} capacity={user.capacity} status={user.status} image={user.image} onEdit={onEdit} onDelete={onDelete}  />
      ))}
    </>
  )
}
